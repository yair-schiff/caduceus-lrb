...
class Lit_DNAModelForCAGE(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.training_step_preds.extend(logits.view(-1, logits.size(-1)).detach().cpu().float().numpy())
        self.training_step_labels.extend(labels.view(-1, labels.size(-1)).detach().cpu().float().numpy())
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # Track predictions and labels for R² score
        self.validation_step_preds.extend(logits.view(-1, logits.size(-1)).detach().cpu().float().numpy())
        self.validation_step_labels.extend(labels.view(-1, labels.size(-1)).detach().cpu().float().numpy())
